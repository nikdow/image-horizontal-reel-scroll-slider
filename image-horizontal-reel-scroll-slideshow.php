...
*/
/*
 * suppress this plugin on mobiles
 */ 
...
function Ihrss() 
{
	...
	if ( ! empty($data) ) 
	{
		foreach ( $data as $data ) 
		{
			...
			$Ihrss_link = get_site_url(); // trim($data->Ihrss_link);
			$Ihrss_target = trim($data->Ihrss_target);
			$Ihrss_title = trim($data->Ihrss_title);
			$Ihrss_package = $Ihrss_package ."IHRSS_SLIDESRARRAY[$cnt]='<a style=\"$Ihrss_slideshowgaphtml\" title=\"$Ihrss_title\" href=\"$Ihrss_link\"><img alt=\"$Ihrss_title\" src=\"$Ihrss_path\" /></a>';";
			$cnt++;
		}
		?>
